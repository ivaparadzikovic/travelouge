...
import { useBrowsePosts, MIN_SEARCH_LENGTH } from '../../api/posts'
...
export default function Browse() {
  ...
  const setCountryFromId = (id: string) => {
  ...
  const setCategoryFromId = (id: string) => {
    const next = new URLSearchParams(searchParams)
  ...
  const onSearchSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (q.length >= MIN_SEARCH_LENGTH) next.set('q', q)
    else next.delete('q')
    setSearchParams(next, { replace: true })
  }
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        <SelectMenu
          ...
          value={countryId}
          onChange={(id) => setCountryFromId(String(id))}
        ...
        {localizedCategories?.map((c) => {
          ...
          return (
            <button
              key={c.id}
              type="button"
              onClick={() => setCategoryFromId(active ? '' : String(c.id))}
              aria-pressed={active}
              className={
                active
